...
void commands(char* command)
{
	...
	if(strncmp("OFF", command, 3) == 0)
	{
		...
		memset(sslBuff, 0, 64);
		sprintf(sslBuff, "%s SHUTDOWN\n", cStringTime);
		SSL_write(ssl, sslBuff, strlen(sslBuff));
		if(logFlag)
		{
			fprintf(logF, "%s\n", command);
			fflush(logF);
...
int main(int argc, char **argv)
{
	...
	{
		fprintf(stderr, "ERROR, unable to build SSL/TLS session\n");
		exit(2);
	}

	//after the server is set up we send over id
	memset(sslBuff, 0, 64);
	sprintf(sslBuff, "ID=%d\n", idNum);
	SSL_write(ssl, sslBuff, strlen(sslBuff));
	if(logFlag)
	{
		fprintf(logF, "ID=%d\n", idNum);
